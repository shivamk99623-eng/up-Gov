...
WHERE
    N."isDeleted" = false
    AND N."postedTime" BETWEEN '2026-06-22 18:30:00' AND '2026-07-10 18:29:59'
`;
...
async function retagLocalNews(props = {}) {
  const rows = await all(
    props.sqliteDb,
    `
    SELECT newsId, Heading, Summary, MLA
    FROM news_x
    WHERE
      MLA LIKE '%Adityanath%'
  ...
  log(`Retagging ${rows.length} local news_x rows...`);
  let updated = 0;
  ...
  log(`✓ Retagged ${updated} news_x rows`);
}
...
async function syncNews() {
  ...
  for (const column of ENTITY_COLUMNS) {
    await ensureColumnExists(sqliteDb, "news_x", column);
  }
